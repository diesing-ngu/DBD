...
```{r load_response}
DBD_data <- read.csv("input/DBD.csv", header = TRUE, sep = ",")
DBD_data <- DBD_data[-c(4,5)]
summary(DBD_data)

names(DBD_data)[5] <- "DBD"

DBD_data <- na.omit(DBD_data) # Removes rows with NAs
...
DBD10 <- subset(DBD, Obs_depth_cm >= brks[2] & Obs_depth_cm < brks[3])
```
...
Centroids are calculated to average all measurements of DBD within one grid cell and the relevant depth interval. Again, this can be executed for 0 - 10 cm and 10 - 30 cm:
...
DBD_centroids <- st_join(fishnet, DBD10)
DBD_centroids <- na.omit(DBD_centroids)
DBD_centroids <- DBD_centroids %>% group_by(ID) %>% summarize(DBD = mean(DBD, na.rm = TRUE))
DBD10 <- st_centroid(DBD_centroids)
```
...
write_sf(DBD10, dsn = "input", layer = paste0("DBD10_", Sys.Date()), driver = "ESRI Shapefile")
```
