...
The variable 'date' specifies the date when the files of the data preparation step were written to the disk.
...
DBD <- read_sf(paste0("input/DBD", du, "_", date, ".shp"))
DBD <- vect(DBD)
...
```{r hist_oc_content}
hist(rm_dbd$DBD, breaks = 40, main = "", xlab = "Dry bulk density (g/cm^3)")
```
...
```{r transformation}
if(nrow(rm_dbd) > 5000) {
  lambda <- transformTukey(sample(rm_dbd$DBD, 5000), plotit = TRUE, quiet = FALSE, returnLambda = TRUE)
  } else {
  lambda <- transformTukey(rm_dbd$DBD, plotit = TRUE, quiet = FALSE, returnLambda = TRUE)
}
```
...
```{r boruta}
set.seed(42)
B <- Boruta(rm_dbd[[1]] ~ .,data=rm_dbd[2:ncol(rm_dbd)], pValue = 0.05,
             maxRuns = 500)
B
...
repeat{
 cor_result<- vifcor(rm_dbd[rownames(subset(attStats(B), decision == "Confirmed"))], th = th,  maxobservations = nrow(rm_dbd))
...
seldata <- rm_dbd[c("DBD", sel_preds)]
```
...
```{r correlation_plot}
corrplot.mixed(cor(rm_dbd[sel_preds]), lower.col =  "black", tl.pos = "lt", number.cex = 0.6)
```
...
```{r}
smp <- as.data.frame(spatSample(x = predictors[[sel_preds]], size = nrow((rm_dbd)), method = "random", na.rm = TRUE))
...
```{r quick_rf}
set.seed(42)
rf <- randomForest(DBD ~ ., seldata, replace = FALSE)
...
DBD_pr <- project(x = DBD, y = "+proj=eqdc +lon_0=12.5 +lat_1=59.5 +lat_2=77.5 +lat_0=68.5 +datum=WGS84 +units=m +no_defs")
DBD_pr <- as(DBD_pr, "Spatial")

if (lambda >  0){
  DBD_pr$DBD = DBD_pr$DBD ^ lambda
} else if (lambda == 0){
    DBD_pr$DBD = log(DBD_pr$DBD)
} else if (lambda <  0){
      DBD_pr$DBD = -1 * DBD_pr$DBD ^ lambda}

# automap package
vf <- autofitVariogram(DBD ~ 1, 
                       DBD_pr, 
                       #cutoff = 5000000
                       )
plot(vf)
...
m <- 3.5 # Multiplier applied to block size

spBlocks <- cv_spatial(x = DBD_pr,
                       k = k,
                       #hexagon = FALSE,
                       size = sar * m,
                       seed = 42,
...
for (i in 1:length(sel_preds)) {
  partialPlot(x = m2, pred.data = seldata, x.var = sel_preds[i], main = "", xlab = sel_preds[i], ylab = "DBD (g/cm^3)")
}
